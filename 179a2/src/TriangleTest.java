...
class TriangleTest {
	...
	static long caculatePerimter(long a, long b, long c) {
    return a + b + c;
  }
	...
	class InvalidTriangleTest{
		...
		@CsvSource(value = {"0,0,0", "-1,-1,-1", "-1,2,3", "1,1,2", "-3,-4,-5", "3,1,1", "2147483647,2147483647,0", "1,1,0"})
	...
	class EquilateralTriangleTest{
		...
		void properties_of_equilateral(int s1, int s2, int s3) {
			...
			assertAll(
				...
				() -> assertEquals(TriangleTest.caculatePerimter(s1, s2, s3), t.getPerimeter(), "equilateral perimeter"),
	...
	class PureIsoscelesTriangleTest{
		@ParameterizedTest(name = "s1 = {0}, s2 = {1}, s3 = {2}")
		@CsvSource(value = {"2,2,1", "1,2,2", "2,1,2", "2147483647, 2147483647, 1", "1, 2147483647, 2147483647", "2147483647, 1, 2147483647"})
		void properties_of_isosceles(int s1, int s2, int s3) {
			...
			assertAll(
				...
				() -> assertEquals(TriangleTest.caculatePerimter(s1, s2, s3), t.getPerimeter(), "pure isosceles perimeter"),
				() -> assertEquals(String.format("%d,%d,%d", s1, s2, s3), t.getSideLengths(), "return input lengths in string"),
	...
	class RightAngledTest{
		...
		void properties_of_equilateral(int s1, int s2, int s3) {
			...
			assertAll(
				...
				() -> assertEquals(true, t.isRightAngled(),"right angled is right angled"),
	...
	class CloseToRightAngledTest{
		...
		@ParameterizedTest(name = "s1 = {0}, s2 = {1}, s3 = {2}")
		@CsvSource(value = {"1295743693, 1295743692, 1832458304", "2147483647, 2147483646, 2"})
		void properties_of_close_to_right_angled(int s1, int s2, int s3) {
			...
			assertAll(
				...
				() -> assertEquals(false, t.isRightAngled(), "close to right angled is not right angled"),
